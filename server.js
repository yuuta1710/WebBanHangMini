...
// ví dụ POST /api/orders vẫn cho khách vãng lai đặt hàng bình thường, nhưng nếu có đăng nhập sẽ gắn kèm user_id)
const optionalAuthenticate = (req, res, next) => {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
        try {
            ...
            // Token sai/hết hạn -> bỏ qua, coi như khách vãng lai, KHÔNG chặn request
        }
    }
    next();
};
...
app.post('/api/orders', strictLimiter, optionalAuthenticate, catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        // 2. Cập nhật câu lệnh INSERT bổ sung cột order_code + user_id (nếu khách đã đăng nhập)
        const insertOrderQuery = `
            INSERT INTO orders (order_code, customer_name, total_price, items, user_id) 
            VALUES ($1, $2, $3, $4, $5) 
            RETURNING id, order_code, customer_name, total_price, status, user_id, created_at;
        `; 
        ...
        const orderResult = await client.query(insertOrderQuery, [
            ...
            req.user ? req.user.id : null   // $5 - ✨ MỚI: gắn user_id nếu khách đã đăng nhập, NULL nếu là khách vãng lai
        ]);
...
app.get('/api/orders/my', authenticate, catchAsync(async (req, res, next) => {
